...
object main extends scala.App {
 ...
 // initialize four actors for testing 
  ...
  // The ActorGroupMessage class is used for creating messages in which the programmer also specifies the number of ticks needed for processing the message. 
  // Ten messages are sent to the four actors in sequence. The system schedules the delivery of these messages according to the allocations of the actorgroups to which the target actors belongs.
  for {i <- 1 to 10} {
    lightActor1  ! new ActorgroupMessage(i, lightActor1, r.nextInt(20), TimeUnit.MILLISECONDS)
    lightActor2  ! new ActorgroupMessage(i, lightActor2, r.nextInt(20), TimeUnit.MILLISECONDS)
    heavyActor1 ! new  ActorgroupMessage(i, heavyActor1, r.nextInt(50), TimeUnit.MILLISECONDS)
    heavyActor2 ! new  ActorgroupMessage(i, heavyActor2, r.nextInt(50), TimeUnit.MILLISECONDS)
  }
...
class LightActor () extends Actor {
   def receive = {
     case m: ActorgroupMessage => {  
        ...
        println("lightActor --> " + m.getMessage()) 
      }
  } 
}
